...
import kotlinx.android.synthetic.main.fragment_fruits_click319.view.*
import java.text.SimpleDateFormat
import java.util.*


class FruitsClick319 : Fragment() {
    private var mPlayer: MediaPlayer? = null
    private val nav = R.id.action_fruitsClick319_to_fruitsSplash34
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_fruits_click319, container, false)
        view.imageViewFruitsKivi5.setOnClickListener {
            mPlayer?.stop()
            mPlayer?.release()
            animation(view.imageViewFruitsKivi5)
            finish=System.currentTimeMillis()
            fark=finish-start
            fark /= 1000

        }
        view.imageViewFruitsKiraz14.setOnClickListener {
            if(mPlayer!=null){
                mPlayer?.stop()
                mPlayer?.release()
            }
            animationWrong(view.imageViewFruitsKiraz14)

        }
        view.imageViewFruitsPortakal14.setOnClickListener {
            if(mPlayer!=null){
                mPlayer?.stop()
                mPlayer?.release()
            }
            animationWrong(view.imageViewFruitsPortakal14)

        }
        view.imageViewFruitsUzum13.setOnClickListener {
            if(mPlayer!=null){
                mPlayer?.stop()
                mPlayer?.release()
            }
            animationWrong(view.imageViewFruitsUzum13)

        }
        view.imageViewFruitsKavun9.setOnClickListener {
            if(mPlayer!=null){
                mPlayer?.stop()
                mPlayer?.release()
            }
            animationWrong(view.imageViewFruitsKavun9)
