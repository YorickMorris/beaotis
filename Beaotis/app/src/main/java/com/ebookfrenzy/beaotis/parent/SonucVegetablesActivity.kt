...
class SonucVegetablesActivity : AppCompatActivity() {
    ...
    private val olumsuz="Çocuğunuzun sebzelerin konusundaki eğitimini tekrarlaması bu aktivitedeki başarısı açısından olumlu sonuçlar doğuracaktır."
                    ...
                    .addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            ...
                            if(basarilar[basarilar.lastIndex]-basarilar[0]>=30f&&basarilar.size>=8){
                                tavsiyeler.add("Çocuğunuz nesneleri tanımada ve öğrendiklerini hatırlama ilk gündeki durumuna göre daha iyi bir sonuç almayı " +
                                        "başarmıştır.")
                                ...
                                if (comparison(basarilar)<=-30f){
                                    tavsiyeler.add("Çocuğunuzun son zamanlarda ilgili aktivitede zorlanıyor olması söz konusu olup sizin gözetiminiz altında " +
                                            "aktiviteyi tekrarlaması yararlı olacaktır. ")
                            ...
                            if(basarilar.size>=8 && basarilar.sum()/basarilar.size>=80f){
                                ...
                                tavsiyeler.add("Çocuğunuz bu aktivitede gösterdiği başarıyı diğer gruplarda da gösteriyorsa ‘Öğrenelim’ kısmındaki aktivitelere " +
                                        "başlayabilir.")
                            ...
                            if(basarilar[basarilar.lastIndex]<=50f){
                                ...
                                tavsiyeler.add("Çocuğunuzun aktiviteleri belli bir süre tekrar etmemesi durumunda bazı nesne ve bilgileri unutma ihtimalleri" +
                                        " bulunmaktadır. Bunun için bu aktiviteyi tekrar etmeleri önemlidir.")
                            }else if(basarilar[basarilar.lastIndex]<=30f){
                                tavsiyeler.add("Çocuğunuzun daha başarılı bir yol katedebilmesi için aktivitedeki sebzelerin kullanımını günlük hayatınızda " +
                                        "kurduğunuz cümlelerde arttırmanız tavsiye edilmektedir. Kullanacağınız cümlelerde karmaşıklık barındırmayıp sadece " +
                                        "öğrenilmesi gereken nesnenin ismini kullanarak bu başarıyı arttırabilirsiniz.")
                                tavsiyeler.add(olumsuz)
                            }

                            val adapter= ArrayAdapter(this, R.layout.sample_text, tavsiyeler)

                            list_viewVegetables.adapter=adapter
