...
class FruitsClick28 : Fragment() {
    ...
    private val nav = R.id.action_fruitsClick28_to_fruitsSplash24
    private val nav2 = R.id.action_fruitsClick28_to_fruitsSplash25
    ...
    override fun onCreateView(
        ...
        view.imageViewFruitsMuz5.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsMuz5)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsCilek5.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsCilek5)
            firebaseYanlisEntry()
        }
            ...
            collection(ab).document(formatDate).get().addOnSuccessListener{
                ...
                if (list != null&&list1!=null) {
                    if(list!!.sum()/ list!!.size<=40&& list1!!.sum()/ list1!!.size<10&&list!!.size>=5){
                        view?.let{it2->animationToFinish(it2.imageViewFruitsKarpuz3,nav2)}
                    }else
                        view?.let { it1 -> animationToFinish(it1.imageViewFruitsKarpuz3,nav) }
                }
