...
import kotlinx.android.synthetic.main.fragment_fruits_click211.view.*
import java.text.SimpleDateFormat
import java.util.*


class FruitsClick211 : Fragment() {
    private var mPlayer: MediaPlayer? = null
    private val nav = R.id.action_fruitsClick211_to_fruitsClick212
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_fruits_click211, container, false)

        view.imageViewFruitsCilek7.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animation(view.imageViewFruitsCilek7)
            finish=System.currentTimeMillis()
            fark=finish-start
            fark /= 1000
            firebaseBitirmeSuresiEntry()
        }
        view.imageViewFruitsMuz7.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsMuz7)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsKarpuz6.setOnClickListener {
            ...
            animationWrong(view.imageViewFruitsKarpuz6)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsMandalina2.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsMandalina2)
            firebaseYanlisEntry()
        }
