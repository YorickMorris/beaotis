...
import kotlinx.android.synthetic.main.fragment_click12.view.imageViewFruitsAnanas7
import kotlinx.android.synthetic.main.fragment_click12.view.imageViewFruitsArmut8
import kotlinx.android.synthetic.main.fragment_click12.view.imageViewFruitsBogurtlen3
import kotlinx.android.synthetic.main.fragment_click12.view.imageViewFruitsElma8
import kotlinx.android.synthetic.main.fragment_fruits_click212.view.*
import java.text.SimpleDateFormat
import java.util.*


class FruitsClick212 : Fragment() {
    private var mPlayer: MediaPlayer? = null
    private val nav = R.id.action_fruitsClick212_to_fruitsClick213
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_fruits_click212, container, false)

        view.imageViewFruitsKarpuz7.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animation(view.imageViewFruitsKarpuz7)
            finish=System.currentTimeMillis()
            fark=finish-start
            fark /= 1000
            firebaseBitirmeSuresiEntry()
        }
        view.imageViewFruitsMuz8.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsMuz8)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsCilek8.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsCilek8)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsMandalina3.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsMandalina3)
            firebaseYanlisEntry()
        }
