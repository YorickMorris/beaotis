...
class Click16 : Fragment() {
    private var mPlayer: MediaPlayer? = null
    private val nav = R.id.action_click16_to_click17
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_click16, container, false)

        view.imageViewFruitsArmut11.setOnClickListener {
            mPlayer?.stop()
            mPlayer?.release()
        ...
        view.imageViewFruitsElma11.setOnClickListener {
            if(mPlayer!=null){
                mPlayer?.stop()
                mPlayer?.release()
            }
            animationWrong(view.imageViewFruitsElma11)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsAnanas10.setOnClickListener {
            if(mPlayer!=null){
                mPlayer?.stop()
                mPlayer?.release()
            }
            animationWrong(view.imageViewFruitsAnanas10)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsBogurtlen6.setOnClickListener {
            if(mPlayer!=null){
                mPlayer?.stop()
                mPlayer?.release()
            }
            animationWrong(view.imageViewFruitsBogurtlen6)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsErik2.setOnClickListener {
            if(mPlayer!=null){
                mPlayer?.stop()
                mPlayer?.release()
            }
            animationWrong(view.imageViewFruitsErik2)
            firebaseYanlisEntry()
        }
