...
import kotlinx.android.synthetic.main.fragment_click10.view.imageViewFruitsAnanas5
import kotlinx.android.synthetic.main.fragment_click10.view.imageViewFruitsArmut6
import kotlinx.android.synthetic.main.fragment_click10.view.imageViewFruitsBogurtlen1
import kotlinx.android.synthetic.main.fragment_click10.view.imageViewFruitsElma6
import kotlinx.android.synthetic.main.fragment_fruits_click210.view.*
import java.text.SimpleDateFormat
import java.util.*


class FruitsClick210 : Fragment() {
    private var mPlayer: MediaPlayer? = null
    private val nav = R.id.action_fruitsClick210_to_fruitsClick211
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_fruits_click210, container, false)

        view.imageViewFruitsMuz6.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animation(view.imageViewFruitsMuz6)
            finish=System.currentTimeMillis()
            fark=finish-start
            fark /= 1000
            firebaseBitirmeSuresiEntry()
        }
        view.imageViewFruitsCilek6.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsCilek6)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsKarpuz5.setOnClickListener {
            ...
            animationWrong(view.imageViewFruitsKarpuz5)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsMandalina1.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsMandalina1)
            firebaseYanlisEntry()
        }
        return view
    }
