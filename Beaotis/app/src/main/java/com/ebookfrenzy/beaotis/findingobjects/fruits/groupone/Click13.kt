...
class Click13 : Fragment() {
    private var mPlayer: MediaPlayer? = null
    private val nav = R.id.action_click13_to_splashFragment4
    private val nav2 = R.id.action_click13_to_splashFragment6
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_click13, container, false)

        view.imageViewFruitsBogurtlen4.setOnClickListener {
        ...
        view.imageViewFruitsElma9.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsElma9)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsArmut9.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsArmut9)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsAnanas8.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsAnanas8)
            firebaseYanlisEntry()
        }
            ...
            collection(ab).document(formatDate).get().addOnSuccessListener{
                ...
                if (list != null&&list1!=null) {
                    if(list!!.sum()/ list!!.size<=55&& list1!!.sum()/ list1!!.size<14&&list!!.size>=5){
                        view?.let{it2->animationToFinish(it2.imageViewFruitsBogurtlen4,nav2)}
                    }else
                        view?.let { it1 -> animationToFinish(it1.imageViewFruitsBogurtlen4,nav) }
                }
