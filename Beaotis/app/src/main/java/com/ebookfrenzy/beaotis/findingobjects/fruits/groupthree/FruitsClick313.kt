...
import kotlinx.android.synthetic.main.fragment_fruits_click313.view.*
import java.text.SimpleDateFormat
import java.util.*


class FruitsClick313 : Fragment() {
    private var mPlayer: MediaPlayer? = null
    private val nav = R.id.action_fruitsClick313_to_fruitsSplash34
    private val nav2 = R.id.action_fruitsClick313_to_fruitsSplash36
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_fruits_click313, container, false)
        view.imageViewFruitsKavun4.setOnClickListener {
        ...
        view.imageViewFruitsKiraz9.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsKiraz9)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsPortakal9.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsPortakal9)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsUzum8.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsUzum8)
            firebaseYanlisEntry()
        }
            ...
            collection(ab).document(formatDate).get().addOnSuccessListener{
                ...
                if (list != null&&list1!=null) {
                    if(list!!.sum()/ list!!.size<=55&& list1!!.sum()/ list1!!.size<14&&list!!.size>=5){
                        view?.let{it2->animationToFinish(it2.imageViewFruitsKavun4,nav2)}
                    }else
                        view?.let { it1 -> animationToFinish(it1.imageViewFruitsKavun4,nav) }
                }
