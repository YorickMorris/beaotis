...
import kotlinx.android.synthetic.main.fragment_fruits_click310.view.*
import java.text.SimpleDateFormat
import java.util.*


class FruitsClick310 : Fragment() {
    private var mPlayer: MediaPlayer? = null
    private val nav = R.id.action_fruitsClick310_to_fruitsClick311
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val view = inflater.inflate(R.layout.fragment_fruits_click310, container, false)

        view.imageViewFruitsKiraz6.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animation(view.imageViewFruitsKiraz6)
            finish=System.currentTimeMillis()
            fark=finish-start
            fark /= 1000
            firebaseBitirmeSuresiEntry()
        }
        view.imageViewFruitsPortakal6.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsPortakal6)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsUzum5.setOnClickListener {
            ...
            animationWrong(view.imageViewFruitsUzum5)
            firebaseYanlisEntry()
        }
        view.imageViewFruitsKavun1.setOnClickListener {
            if(mPlayer!=null&& mPlayer!!.isPlaying){
                mPlayer?.stop()
                mPlayer?.release()
                mPlayer=null
            }
            animationWrong(view.imageViewFruitsKavun1)
            firebaseYanlisEntry()
        }
